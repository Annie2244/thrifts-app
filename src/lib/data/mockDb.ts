...
export function mockUpsertSellerByName(name: string): Seller {
  const db = loadDb();
...
export function mockListProducts(filters?: MarketplaceFilters): Product[] {
  const db = loadDb();
...
export function mockCreateProduct(input: {
  sellerName: string;
  title: string;
  description: string;
  price: number;
  ...
  const db = loadDb();
  const seller = mockUpsertSellerByName(input.sellerName);
...
export function mockIncrementProductViews(productId: string) {
  const db = loadDb();
...
export function mockListOrdersByBuyer(buyerName: string): Order[] {
  const db = loadDb();
...
export function mockListOrdersBySeller(sellerName: string): Order[] {
  const db = loadDb();
...
export function mockInbox(userName: string) {
  const db = loadDb();
...
export function mockThread(threadKey: string) {
  const db = loadDb();
...
export function mockSellerStats(sellerName: string) {
  const db = loadDb();
  const seller = db.sellers.find(
    (s) => s.name.toLowerCase() === sellerName.toLowerCase()
  );
...
export function mockMostCheckedToday(limit = 10): Product[] {
  const db = loadDb();
  return [...db.products]
...
export function mockListSellerProducts(sellerName: string): Product[] {
  const db = loadDb();
